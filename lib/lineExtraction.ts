...
const CFG: Record<BaseLine, TraceCfg> = {
  ...
  heart_line: { along: linspace(1.25, -0.15, 15), searchAxis: "v", range: [0.04, 0.32], steps: 26, expected: () => 0.175, sigma: 0.06 },
  // 知能線：基準図に合わせ起点を親指‑人差し指の谷(生命線と共有, u≈-0.1)へ。右下がりに横断。
  head_line: { along: linspace(-0.12, 0.95, 14), searchAxis: "v", range: [0.16, 0.5], steps: 26, expected: () => 0.35, sigma: 0.09 },
  // 運命線：中央を縦に（v に沿い u を探索）。検証済み推奨 u=0.48〜0.52 の中央へ。
  fate_line: { along: linspace(0.95, 0.18, 14), searchAxis: "u", range: [0.28, 0.64], steps: 26, expected: () => 0.5, sigma: 0.1 },
